...
class Deform_Model(nn.Module):
    def __init__(self, device):
        ...
        uv_coords = uv_coords * 2 - 1 # (1, 5150, 2)
        uv_coords[..., 1] = - uv_coords[..., 1]
        self.uvcoords = torch.cat([uv_coords, uv_coords[:, :, 0:1] * 0. + 1.], -1).to(self.device) # (1, 5150, 3)最后一维全为1.0
    ...
    def example_init(self, codedict):
        ...
        ) # (1, 5023, 3) 5023个顶点，每个顶点的坐标值

        face_vertices_shape = face_vertices_gen(geometry_shape, self.tri_faces.expand(batch_size, -1, -1)) # (1, 10006, 3, 3) 10006个面片，每个面片对应3个顶点
        rast_out, pix_to_face, bary_coords = self.uv_rasterizer(self.uvcoords.expand(batch_size, -1, -1),
                                         ...
                                         face_vertices_shape) # rast_out (1, 4, 128, 128) pix_to_face (1, 128, 128, 1) bary_coords (1, 128, 128, 1, 3)
        self.pix_to_face_ori = pix_to_face
        self.bary_coords = bary_coords

        uvmask = rast_out[:, -1].unsqueeze(1) # (1, 1, 128, 128)
        uvmask_flaten = uvmask[0].view(uvmask.shape[1], -1).permute(1, 0).squeeze(1) # batch=1 (16384)
        self.uvmask_flaten_idx = (uvmask_flaten[:]>0) # (16384) bool类型的数组，像素点有映射关系的值为True

        pix_to_face_flaten = pix_to_face[0].clone().view(-1) # batch=1 (16384)
        self.pix_to_face = pix_to_face_flaten[self.uvmask_flaten_idx] # pix to face idx (14876)
        self.pix_to_v_idx = self.tri_faces[0, self.pix_to_face, :] # pix to vert idx (14876, 3)

        uv_vertices_shape = rast_out[:, :3] # uv pixel value ，第四维是mask
    ...
    def decode(self, codedict, use_xyz_offset=True):
        ...
        face_vertices = face_vertices_gen(geometry, self.tri_faces.expand(batch_size, -1, -1)) # (1, 10006, 3, 3)

        # rasterize face_vertices to uv space
        D = face_vertices.shape[-1] # 3
        attributes = face_vertices.clone()
        attributes = attributes.view(attributes.shape[0] * attributes.shape[1], 3, attributes.shape[-1]) # (10006, 3, 3)
        N, H, W, K, _ = self.bary_coords.shape # (1, 128, 128, 1, 3)
        idx = self.pix_to_face_ori.clone().view(N * H * W * K, 1, 1).expand(N * H * W * K, 3, D) # (16384, 1, 1) -> (16384, 3, 3)
        pixel_face_vals = attributes.gather(0, idx).view(N, H, W, K, 3, D) # 找到每个像素点对应的mesh上的顶点坐标（根据face找到）
        pixel_vals = (self.bary_coords[..., None] * pixel_face_vals).sum(dim=-2) # 重心插值得到每个像素点对应的点的坐标 (1, 128, 128, 1, 3)
        uv_vertices = pixel_vals[:, :, :, 0].permute(0, 3, 1, 2) # (1, 3, 128, 128)
        uv_vertices_flaten = uv_vertices[0].view(uv_vertices.shape[1], -1).permute(1, 0) # batch=1 (16384, 3)
        uv_vertices = uv_vertices_flaten[self.uvmask_flaten_idx].unsqueeze(0) # (1, 14876, 3)
